/**
 * Security, Anti-Bypass Contact Masking, Rate Limiting, Fraud Prevention, and RBAC Engine
...
/**
 * Rate Limiting Guard (Default: 30 requests per minute per IP / Identifier)
 */
export function checkRateLimit(
  ...
  entry.count += 1;
  return { allowed: true, remaining: maxRequests - entry.count, resetMs: entry.expiresAt - now };
}

/**
 * Input XSS & SQL Injection Sanitizer
 */
...
export function sanitizeContentForBypass(
  ...
  if (phoneMatches) detectedPhones.push(...phoneMatches);

  const emailMatches = text.match(EMAIL_REGEX);
  if (emailMatches) detectedEmails.push(...emailMatches);

  const keywordMatches = text.match(OFF_PLATFORM_KEYWORDS_REGEX);
  if (keywordMatches) detectedKeywords.push(...keywordMatches);
...
export function sanitizeBookingContacts<T extends Record<string, any>>(
  ...
  if (!isPaid) {
    if (sanitized.phone) sanitized.phone = "[LOCKED UNTIL PAYMENT]";
    if (sanitized.customerPhone) sanitized.customerPhone = "[LOCKED UNTIL PAYMENT]";
    if (sanitized.proPhone) sanitized.proPhone = "[LOCKED UNTIL PAYMENT]";
    if (sanitized.exactStreetAddress) sanitized.exactStreetAddress = "[STREET ADDRESS MASKED UNTIL PAYMENT CONFIRMED]";
  }
